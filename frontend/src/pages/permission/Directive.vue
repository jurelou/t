<template>
  <base-content scrollable padding>
    <q-card-section class="q-gutter-md">
      ...
      <div class="text-h6">
        {{ 'permission.currentPermission' }}: {{ userStore.getUserRoles }}
      </div>
      <div class="text-subtitle2"> {{ 'permission.description' }}</div>
    </q-card-section>
    <q-card-section class="q-gutter-md">
      <div class="text-h6">v-permission</div>
      <div class="q-gutter-sm">
        <q-btn v-permission="['admin', 'user']" unelevated color="primary"
          :label="`admin & user ${'permission.canSee'}`" />
        <q-btn v-permission="['admin']" unelevated color="primary" :label="`admin ${'permission.canSee'}`" />
        <q-btn v-permission="['user']" unelevated color="primary" :label="`user ${'permission.canSee'}`" />
      </div>
    </q-card-section>
    <q-card-section class="q-gutter-md">
